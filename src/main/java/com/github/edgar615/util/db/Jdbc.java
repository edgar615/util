...
public interface Jdbc {
   ...
   * @return 修改记录数
   */
  default <ID> int updateByMoreExample(Persistent<ID> persistent, MoreExample example) {
   ...
   * @return 分页对象
   */
  default <ID, T extends Persistent<ID>> Pagination<T> paginationMoreExample(Class<T> elementType,
      MoreExample example,
      int page, int pageSize) {
    Preconditions.checkArgument(page > 0, "page must greater than 0");
    Preconditions.checkArgument(pageSize > 0, "pageSize must greater than 0");
    ...
    int offset = (page - 1) * pageSize;
    List<T> records = findByMoreExample(elementType, example, offset, pageSize);
    return Pagination.newInstance(page, pageSize, totalRecords, records);
  }
   ...
   * @return 持久化对象，如果未找到任何数据，返回null
   */
  default <ID, T extends Persistent<ID>> T findFirstByMoreExample(Class<T> elementType,
      MoreExample example) {
    List<T> result = findByMoreExample(elementType, example, 0, 1);
    if (result == null || result.isEmpty()) {
      return null;
    }
    return result.get(0);
   ...
   * @return 分页对象
   */
  default <ID, T extends Persistent<ID>> Page<T> pageMoreExample(Class<T> elementType,
      MoreExample example,
      int start,
      int limit) {
    List<T> records = findByMoreExample(elementType, example, start, limit);
    //如果records的数量小于limit，说明已经没有记录，直接计算总数
    if (records.size() > 0 && records.size() < limit) {
      int total = start + records.size();
      return Page.newInstance(total, records);
   ...
   * @return 删除记录数
   */
  <ID, T extends Persistent<ID>> int deleteByMoreExample(Class<T> elementType, MoreExample example);
   ...
   * @return 修改记录数
   */
  <ID> int updateByMoreExample(Persistent<ID> persistent,
      Map<String, Number> addOrSub,
   ...
   * @return 记录数
   */
  <ID, T extends Persistent<ID>> int countByMoreExample(Class<T> elementType, MoreExample example);
